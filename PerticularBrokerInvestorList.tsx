...
import { investorsDetailsValidate } from '../../../config/Validate';

export const PerticularBrokerInvestorList = () => {
    document.title = "Investors | Venture Studio"
    ...
    const getInvestorsDataFromAPI = async (state: any) => {
        // Api call
        pageService
            .getClientsList(status.code, "investor", state.broker_id)
            .then((response) => {
                // Get response
                if (response) {
                    const DataList = response;
    ...
    const approveRejectClient = async () => {
        ...
        pageService.approveRejectClient(formData).then((response) => {
            ...
            if (response) {
                ...
                getInvestorsDataFromAPI(brokerId);
            } else {
                setSubmitLoading(false);
    ...
    const actionBodyTemplate = (rowData: any) => {
        return (
            <div className="tb-actions">
                ...
                <Button
                    icon="pi pi-check"
                    className="p-button-square p-button-outlined mr-2"
                ...
                <Button
                    icon="pi pi-eye"
                    className="p-button-square p-btn-default"
                    onClick={() => navigate('/investor/details', { state: { investor_id: rowData.id } })}
                />
            </div>
        );
    };
            ...
            <>
                <span className="tb-avatar-box">
                    {rowData?.profile_img != null && rowData?.profile_img != "null" && rowData?.profile_img != "" ?
                        <Avatar className="tb-avatar-img" image={rowData?.profile_img} shape="circle" /> : <Avatar className='tb-avatar-img user-list-avatar' label={rowData?.full_name.charAt(0).toUpperCase()} shape="circle" />}<div className="tb-avatar-info"><div className="tb-avatar-name">{rowData?.full_name}</div></div>
                </span>
            </>
        )
    };
    ...
    return (
        <>
            ...
            <div className="page-header">
                <div className="page-leftheader">
                    <div className="page-header-info">
                        <div className="page-title"><Button icon="pi pi-arrow-left" className="link-prev-btn" onClick={() => navigate(-1)} />Investors</div>
            ...
            <Dialog
                ...
                style={{ width: '450px' }}
                header={!window.cn(approveRejectStatus) && approveRejectStatus == "Approved" ? "Approve Investor" : "Revoke Investor Access"}
                modal
                footer={
                    <>
                        <Button
